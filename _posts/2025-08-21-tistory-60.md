...
데이터에서 최소표본 크기 $s$개를 무작위로 뽑아 모델 $\theta$를 추정하고, 에러 $e(x,\theta)$가 임곗값 $\tau$ 이하인 점들의 집합 $\mathcal I(\theta)$ 크기를 평가한다. 가장 큰 $\lvert \mathcal I \rvert$를 주는 $\theta^*$를 기록하면서 반복하고, 마지막에 $\mathcal I(\theta^*)$만으로 재추정(refit)해 정밀도를 높인다.
