...
slug: prob-26-sum-independent-continuous-rvs
...
$$
\begin{aligned}f_X(w) \ast f_Y(w)\end{aligned}
$$
...
$$
(f_X\ast f_Y)(w)=\int_{-\infty}^{\infty} f_X(w-\tau)\,f_Y(\tau)\,d\tau
$$

따라서 Independence 하에서 $W=X+Y$의 PDF는 $f_W(w)=(f_X\ast f_Y)(w)$로 정리된다.
...
- $n$개의 independent continuous random variable 합 $S=\sum_{i=1}^n X_i$의 PDF는 $f_S=f_{X_1}\ast f_{X_2}\ast \cdots\ast f_{X_n}$로 반복 Convolution으로 계산된다.
